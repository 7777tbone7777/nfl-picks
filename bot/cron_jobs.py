...
from .time_utils import now_utc, to_naive_utc, is_tuesday_local
...
def _find_upcoming_week_row(season: int, now_naive_utc: datetime) -> Optional[dict]:
    return db.session.execute(_text('''
        ...
        LIMIT 1
    '''), {"s": season, "now": now_naive_utc}).mappings().first()

def cron_import_upcoming_week() -> Dict[str, Any]:
    cfg = load_config()
    app = create_app()
    with app.app_context():
        allow_anyday = (os.getenv("ALLOW_ANYDAY") or "").strip().lower() in ("1","true","yes","y")
        utc_now = now_utc()
        ...
        import asyncio
        events = asyncio.run(fetch_week(week, season, retries=cfg.espn_retries, backoff_s=cfg.espn_backoff_s))

        if not events:
            asyncio.run(notify_admins(cfg.telegram_bot_token, cfg.admin_chat_ids,
        ...
        row = db.session.execute(_text('''
            ...
            RETURNING id
        '''), {"y": season, "w": week}).first()
        if row:
            week_id = row[0]
        else:
            week_id = db.session.execute(_text(
                "SELECT id FROM weeks WHERE season_year=:y AND week_number=:w"
        ...
        for ev in events:
            ...
            '''), {
                "game_time": start_dt, "status": status,
                "home_score": ev.get("home_score"), "away_score": ev.get("away_score"),
                "week_id": week_id, "home": home, "away": away,
            })
            if res.rowcount == 0:
        ...
        return {"status": "imported", "season_year": season, "week": week,
                "games": int(count_after or 0), "created": created, "updated": updated}
...
def cron_syncscores_latest_active() -> Dict[str, Any]:
    ...
    with app.app_context():
        ...
        if row:
            ...
            target_week = int(db.session.execute(_text('''
                SELECT COALESCE(MAX(week_number),1) FROM weeks WHERE season_year=:y
            '''), {"y": season}).scalar() or 1)

        import asyncio
        events = asyncio.run(fetch_week(target_week, season, retries=cfg.espn_retries, backoff_s=cfg.espn_backoff_s))
        ...
        rows = db.session.execute(_text('''
            ...
            WHERE w.season_year=:y AND w.week_number=:w
        '''), {"y": season, "w": target_week}).mappings().all()
        ...
        for r in rows:
            ...
            new_status = {"pre":"scheduled","in":"in_progress","post":"final"}.get(ev.get("state") or "", "scheduled")
            new_home = ev.get("home_score")
            new_away = ev.get("away_score")

            if new_status and new_status != r["status"]:
                db.session.execute(_text("UPDATE games SET status=:s WHERE id=:id"),
                                   {"s": new_status, "id": r["id"]})
                updated_status += 1
                changed += 1

            if (new_home is not None and new_home != r["home_score"]) or (new_away is not None and new_away != r["away_score"]):
                db.session.execute(_text("UPDATE games SET home_score=:hs, away_score=:as WHERE id=:id"),
